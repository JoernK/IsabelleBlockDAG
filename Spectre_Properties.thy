...
proof(induction G a b c rule: vote_Spectre.induct)
  ...
  next
    ...
    then have a_not_gen: "\<not> blockDAG.is_genesis_node G a"
      ...
      by metis
    then have bD: "blockDAG (reduce_past G a)" using blockDAG.reduce_past_dagbased 
        three by auto
    have b_in2: "b \<in> past_nodes G a" using three by auto
    also have c_in2: "c \<in> past_nodes G a" using three by auto
    ultimately have "c \<rightarrow>\<^sup>+\<^bsub>reduce_past G a\<^esub> b" using DAG.reduce_past_path2 three 1
      by (metis blockDAG.axioms(1)) 
    then have allsorted01:"\<forall>x. x \<in> set (sorted_list_of_set (past_nodes G a)) \<longrightarrow>
          vote_Spectre (reduce_past G a) x b c \<in> {0, 1}" using 1 three by auto
    then have all01: "\<forall>x. x \<in> (past_nodes G a) \<longrightarrow>
          vote_Spectre (reduce_past G a) x b c \<in> {0, 1}"
      using  subs(1) three sorted_list_of_set(1) DAG.finite_past
      by metis 
    obtain wit where wit_in: "wit \<in> past_nodes G a" 
      and wit_vote: "vote_Spectre (reduce_past G a) wit b c \<noteq> 0"
      using vote_Spectre_one_exists b_in2 c_in2 bD induce_subgraph_verts reduce_past.simps
      by metis 
    then have wit_vote1: "vote_Spectre (reduce_past G a) wit b c = 1" using all01
      by blast 
    obtain the_map where the_map_in: 
      "the_map = (map (\<lambda>i. vote_Spectre (reduce_past G a) i b c) 
    ...
    have "\<exists>x \<in> set the_map. x = 1"
      ...
      using wit_in wit_vote1
        sorted_list_of_set(1) DAG.finite_past bD subs(1)
    ...
    then have "tie_break_int b c (signum (sum_list the_map)) = 1" using tie_break_int.simps
      by simp
    then have "vote_Spectre G a b c = 1 " unfolding the_map_in using three vote_Spectre.simps
      by simp
    then show ?thesis by simp
  next
    case four 
    then have all01: "\<forall>a2. a2 \<in> set (sorted_list_of_set (future_nodes G a)) \<longrightarrow>
                              vote_Spectre G a2 b c \<in> {0,1}"
      using 1
      by metis
    have "\<forall>a2. a2 \<in> set (sorted_list_of_set (future_nodes G a)) 
                \<longrightarrow> vote_Spectre G a2 b c \<ge> 0" 
    proof safe
      fix a2
      assume " a2 \<in> set (sorted_list_of_set (future_nodes G a))"
      then have "vote_Spectre G a2 b c \<in> {0, 1}" using all01 by auto
      then show "vote_Spectre G a2 b c  \<ge> 0"
        by fastforce
    qed 
    then have "(sum_list (map (\<lambda>i. vote_Spectre G i b c) 
      (sorted_list_of_set (future_nodes G a)))) \<ge> 0" using sum_list_mono sum_list_0
      by metis 
    then have "signum (sum_list (map (\<lambda>i. vote_Spectre G i b c) 
      (sorted_list_of_set (future_nodes G a)))) \<in> {0,1}" unfolding signum.simps
      by simp 
...
proof - 
  ...
  have "(vote_Spectre G wit a b) \<in> set the_map" 
    ...
    using B.blockDAG_axioms fin_digraph.finite_verts 
      subs(3) sorted_list_of_set(1) wit_in image_iff
    by metis 
...
lemma vote_Spectre_antisymmetric: 
  shows "b \<noteq> c \<Longrightarrow> vote_Spectre G a b c = - (vote_Spectre G a c b)"
proof(induction G a b c rule: vote_Spectre.induct)
  case (1 G a b c)
  show "vote_Spectre G a b c = - vote_Spectre G a c b"
  proof(cases a b c G rule:Spectre_casesAlt)
    case no_bD
 ...
 (vote_Spectre (reduce_past G a) i b c)) (sorted_list_of_set (past_nodes G a)))))"
    ...
    then have "vote_Spectre G a c b = - (tie_break_int b c (signum (sum_list (map (\<lambda>i.
    (vote_Spectre (reduce_past G a) i b c)) (sorted_list_of_set (past_nodes G a))))))" 
   ...
   (vote_Spectre G i b c)) (sorted_list_of_set (future_nodes G a))))"
    ...
    have "(map (\<lambda>i. - vote_Spectre G i b c) (sorted_list_of_set (future_nodes G a)))
     = (map uminus (map (\<lambda>i. vote_Spectre G i b c) (sorted_list_of_set (future_nodes G a))))" 
      using map_map by auto       
    then have "vote_Spectre G a c b = - ( signum (sum_list (map (\<lambda>i.
    (vote_Spectre G i b c)) (sorted_list_of_set (future_nodes G a)))))" 
...
lemma vote_Spectre_reflexive:
  assumes "blockDAG G"
    and "a \<in> verts G"
  shows "\<forall>b \<in> verts G. vote_Spectre G b a a = 1 " using vote_Spectre.simps assms by auto 

lemma Spectre_Order_reflexive:
  assumes "blockDAG G"
    and "a \<in> verts G" 
  shows "Spectre_Order G a a" 
  unfolding Spectre_Order_def 
proof -   
  obtain l where l_def: "l = (map (\<lambda>i. vote_Spectre G i a a) (sorted_list_of_set (verts G)))"
    by auto
  have only_one: "l = (map (\<lambda>i.1) (sorted_list_of_set (verts G)))"
  ...
  have "sum_list l = card (verts G)" using ne only_one sum_list_map_eq_sum_count
  ...
  then show "tie_break_int a a
    (signum (sum_list (map (\<lambda>i. vote_Spectre G i a a) (sorted_list_of_set (verts G))))) = 1"
...
lemma Spectre_Order_antisym: 
  assumes "blockDAG G"
    ...
    and "b \<in> verts G" 
    and "a \<noteq> b"
  shows "Spectre_Order G a b = (\<not> (Spectre_Order G b a))"
proof -
  obtain wit where wit_in: "vote_Spectre G wit a b \<noteq> 0 \<and> wit \<in> verts G" 
    using  vote_Spectre_one_exists assms
    by blast 
  obtain l where l_def: "l = (map (\<lambda>i. vote_Spectre G i a b) (sorted_list_of_set (verts G)))"
    by auto
  have "wit \<in>  set (sorted_list_of_set (verts G))" 
    using  wit_in sorted_list_of_set(1) 
      fin_digraph.finite_verts subs
    by (simp add: fin_digraph.finite_verts subs assms(1)) 
  then have "vote_Spectre G wit a b \<in> set l" unfolding l_def
    by (metis (mono_tags, lifting) image_eqI list.set_map)
  then have dm: "tie_break_int a b (signum (sum_list l)) \<in> {-1,1}"
    by auto  
  obtain l2 where l2_def: "l2 = (map (\<lambda>i. vote_Spectre G i b a) (sorted_list_of_set (verts G)))"
    by auto
...
lemma Spectre_Order_total:
  assumes "blockDAG G"
    and "a \<in> verts G \<and> b \<in> verts G" 
  shows "Spectre_Order G a b \<or> Spectre_Order G b a"
proof safe
  assume notB: " \<not> Spectre_Order G b a"
  consider (eq) "a = b"| (neq) "a \<noteq> b" by auto
  then show "Spectre_Order G a b"
  proof (cases)
    case eq
